...
import { randomFigureId, randomHeadingId } from "../../schema/common/index.js";
...
export class DocxConvert {
    ...
    convertDocument(body) {
        ...
        document.attrs.title =
            title.content.map((node) => node.text || "").join("") ||
                gettext("Untitled");
    ...
    extractAbstract(body) {
        ...
        if (abstractNodes.length) {
            return {
                content: abstractNodes
                    .map((abstractNode) => this.convertBlock(abstractNode))
                    .filter((b) => b !== null),
        ...
        if (extractedPart.content.length) {
            return {
                content: extractedPart.content
                    .map((abstractNode) => this.convertBlock(abstractNode))
                    .filter((b) => b !== null),
                containerNodes: extractedPart.content.concat(extractedPart.header ? [extractedPart.header] : [])
            };
        }
        return { content: [], containerNodes: [] };
    }
    ...
    extractPartOnTitle(body, titleWords, maxPars = false) {
        ...
        if (header && header.nextSibling) {
            ...
            while (searchPar.nextSibling &&
                ...
                if (paraStyle.isHeading && (paraStyle.level ?? 0) <= (headerLevel ?? 0)) {
                    break;
                }
    ...
    getSectionTitle(node, style) {
        ...
        if (style.isHeading && (style.level ?? 0) <= 4) {
    ...
    getParaStyle(node) {
        ...
        const ilvl = parseInt(attr(numPr?.query("w:ilvl"), "w:val") || "0");
        return {
            ...style,
            numbering: numId
                ? {
    ...
    convertFigure(node, captionNode = null) {
        ...
        const imagePath = rel.target.split("/").pop() || rel.target;
    ...
    renderField(field) {
        ...
        // Handle citation fields
            ...
            const cite = parseDocxFieldCitation(instr, field.data, this.sourcesXml, this.bibliography);
            return cite ? [cite] : [];
        }
    ...
    createMarksFromFormatting(formatting, insertion = null, deletion = null) {
        ...
        if (formatting.fontFamily) {
            ...
            const isMonospace = monospacePatterns.some((pattern) => pattern.test(String(formatting.fontFamily)));
            if (isMonospace) {
    ...
    getCurrentMarks(formatting, insertion = null, deletion = null) {

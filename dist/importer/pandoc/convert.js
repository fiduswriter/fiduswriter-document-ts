...
function isStr(inline) {
    return inline.t === "Str";
}
function isNote(inline) {
...
export class PandocConvert {
    ...
    init() {
        try {
            ...
            throw new Error("Invalid Pandoc document format: " +
    ...
    convert() {
        ...
        document.content.push({
            type: "title",
            content: this.convertInlines(this.metaInlines("title") || [{ t: "Str", c: "Untitled" }])
        });
        ...
        switch (block.t) {
            ...
            case "Div":
                ...
                if (block.c[0][1].includes("figure")) {
                    const figureBlock = (block.c[1] || []).find((b) => b.t === "Figure");
                    if (figureBlock) {
            ...
            case "DefinitionList": {
                return block.c.flatMap(item => [
                    {
                        type: "paragraph",
                        content: applyMarkToNodes(this.convertInlines(item[0]), "strong")
                    },
                    {
                        type: "bullet_list",
                ...
                if (attrs && attrs.startsWith("ZOTERO_ITEM CSL_CITATION")) {
                    try {
                        ...
                        const citations = cslData.citationItems.map(item => {
                            ...
                            let [bibKey] = Object.entries(this.bibliography).find(([, entry]) => entry.entry_key === id) || [];
                            if (!bibKey) {
    ...
    convertCitation(cite) {
        const references = cite.c[0]
            ...
            const foundEntry = Object.entries(this.bibliography).find(([, definition]) => definition.entry_key === ref.citationId);
            if (!foundEntry) {
                return;
            }

...
export class OdtConvert {
        ...
        changedRegions.forEach(region => {
            ...
            if (changeInfo) {
                ...
                if (deletion) {
                    ...
                    deletions[id] = deletion.children.filter(isElement).filter(child => child.tagName !== "office:change-info");
                }
            }
        });
    ...
    convertFootnote(node, currentStyleMarks) {
        ...
        if (referenceMarkStart &&
            ...
            firstParagraph.children.filter(isElement).every((child) => child.tagName === "text:reference-mark-start" ||

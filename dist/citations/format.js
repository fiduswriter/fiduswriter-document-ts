...
export class FormatCitations {
    ...
    get bibHTML() {
        ...
        const bib = this.bibliography, bibHTML = bib[0].bibstart + bib[1].join("") + bib[0].bibend;
    ...
    process(citeprocInstanceRaw) {
        const citeprocInstance = citeprocInstanceRaw;
        const allIds = [];
        this.citations.forEach(cit => cit.citationItems.forEach(item => allIds.push(String(item.id))));
        ...
        for (let i = 0; i < len; i++) {
            const citation = this.citations[i], citationTexts = citeprocInstance.appendCitationCluster(citation, true);
            if (inText && "textcite" == this.bibFormats[i]) {
                ...
                const target = citationTexts.find(citationText => citationText[0] === i);
                if (target) {

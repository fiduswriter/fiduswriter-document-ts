...
export class DOCXExporter {
    ...
    init() {
        ...
        const metadata = new DOCXExporterMetadata(xml, this.getBaseMetadata(), this.csl);
        ...
        const footnotes = new DOCXExporterFootnotes(this.doc, this.docContent, this.doc.settings, this.imageDB, this.bibDB, xml, citations, this.csl, lists, math, tables, rels);
        const richtext = new DOCXExporterRichtext(this.doc, this.doc.settings, lists, footnotes, math, tables, rels, citations, images);
        ...
        return {
            ...
            keywords: (this.docContent.content || []).reduce((keywords, part) => {
                if (part.type === "tags_part" &&
                    ...
                    part.content) {
                    return keywords.concat(part.content.map(keywordNode => String(keywordNode.attrs?.tag)));
                }
                else {
                    return keywords;
                }

...
export class DOCXExporterRender {
    xml;
    filePath; // "word/document.xml" or "word/document2.xml" in some cases
    ctXML;
    text;
    constructor(xml) {
        this.xml = xml;
    ...
    init() {
        return this.xml
            ...
            this.filePath = String(documentOverride.getAttribute("PartName")).slice(1);
            ...
            const ignorable = [
                ...new Set(["w14", "wp14", "w15"].concat(String(documentEl.getAttribute("mc:Ignorable") || "")
                    .split(" ")
    ...
    parseStructuredTags(block, tag) {
        ...
        blockText = this.processConditionals(blockText, {
            ...
            content: (tag.content && Array.isArray(tag.content) ? tag.content : [])
        });
    ...
    processLoop(templateXml, items, tagName, limit = null) {
        ...
        effectiveItems.forEach((item, index) => {
            ...
            if (typeof item === "string") {
                ...
                itemXml = itemXml
                    .replace(/\{?%firstname\}?/g, escapeText(contributor.firstname || ""))
                    .replace(/\{?%lastname\}?/g, escapeText(contributor.lastname || ""))
                    .replace(/\{?%institution\}?/g, escapeText(contributor.institution || ""))
                    .replace(/\{?%email\}?/g, escapeText(contributor.email || ""))
                    .replace(/\{?%id_type\}?/g, escapeText(contributor.id_type || ""))
                    .replace(/\{?%id_value\}?/g, escapeText(contributor.id_value || ""));
            }
    ...
    _replaceMultiBlockLoop(blocks, beginIndex, endIndex, tag, limit) {
        ...
        const newBlocks = root.children.filter(child => child.tagName === "w:p" ||
            child.tagName === "w:sectPr");
    ...
    getTagData(docContent, pmBib, settings) {
        ...
        tags.push({
            ...
            content: copyright && copyright.holder
                ? [
                    {
                        type: "paragraph",
                        content: [
        ...
        tags.push({
            ...
            content: copyright && copyright.licenses.length
    ...
    render(docContent, pmBib, settings, richtext, citations) {
        ...
        blocks.forEach(block => {
            ...
            if (pageSize && pageMargins) {
                ...
                let width = Number.parseInt(String(pageSize.getAttribute("w:w"))) -
                    Number.parseInt(String(pageMargins.getAttribute("w:right"))) -
                    Number.parseInt(String(pageMargins.getAttribute("w:left")));
                const height = Number.parseInt(String(pageSize.getAttribute("w:h"))) -
                ...
                const colCount = cols
                    ...
                    : 1;
                if (cols && colCount > 1) {
                    const colSpace = Number.parseInt(String(cols.getAttribute("w:space")));
    ...
    blockRender(tag, citations, richtext) {
        ...
        const outXML = content
            ? content
                .map((contentNode, i) => richtext.run(contentNode, options, content[i + 1]))
                .join("")
            : "";
        if (!outXML.length) {

...
export class DOCXExporterRichtext {
    ...
    commentRangeCounter;
    changeCounter;
    fnCounter; // footnotes 0 and 1 are occupied by separators by default.
    bookmarkCounter;
    categoryCounter; // counters for each type of figure (figure/table/photo)
    fncategoryCounter;
    docPrCount;
    citationCounter; // Track which citation we're processing
    paragraphIdCounter; // Used for w14:paraId attributes on comments
    ...
    transformRichtext(node, options = {}, nextNode = null) {
        ...
        if (inlineType) {
            const inlineInsert = node.marks?.find((mark) => mark.type === "insertion" &&
                ...
                options.blockInsert;
            inlineDelete =
                node.marks?.find((mark) => mark.type === "deletion")
                    ?.attrs ||
                    options.blockDelete;
            if (inlineInsert &&
                inlineDelete &&
        ...
        switch (node.type) {
            ...
            case "code_block": {
                ...
                if (typeof category === "string" &&
                    ...
                    const categoryLabelText = getCat(category, this.settings.language);
            ...
            case "citation": {
                ...
                if (options.citationType === "note" && !options.inFootnote) {
                    ...
                    const footnotesXml = this.footnotes.xml.docs[this.footnotes.filePath];
                    if (!footnotesXml) {
                    ...
                    footnotes.forEach((footnote) => {
                        const id = Number.parseInt(String(footnote.getAttribute("w:id")));
            ...
            case "table": {
                ...
                options.dimensions = Object.assign({ width: 0, height: 0 }, options.dimensions);

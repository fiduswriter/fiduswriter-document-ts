...
export class DOCXExporterComments {
    ...
    init() {
        ...
        if (useExtended) {
            ...
            addCommentXMLs.push(this.xml
                .getXml(this.commentsExtendedFilePath, DEFAULT_COMMENTS_EXTENDED_XML)
    ...
    addComment(id) {
        ...
        let string = `<w:comment w:id="${commentId}" w:author="${escapeText(commentDBEntry.username)}" w:date="${new Date(commentDBEntry.date).toISOString().split(".")[0]}Z" w:initials="${escapeText(commentDBEntry.username
            .split(" ")
            .map(n => n[0])
            .join("")
            .toUpperCase())}">`;
        ...
        commentDBEntry.answers?.forEach(answer => {
            ...
            string += `<w:comment w:id="${answerId}" w:author="${escapeText(answer.username)}" w:date="${new Date(answer.date).toISOString().split(".")[0]}Z" w:initials="${escapeText(answer.username
                .split(" ")
                .map(n => n[0])
                .join("")
                .toUpperCase())}">`;

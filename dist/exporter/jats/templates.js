...
export const articleTemplate = ({ front, body, back }) => `<?xml version="1.0" encoding="UTF-8"?>
...
export const bookPartWrapperTemplate = ({ front, body, back }) => `<?xml version="1.0" encoding="UTF-8"?>
...
export const darManifest = ({ images, title, type }) => noSpaceTmp `<?xml version="1.0" encoding="UTF-8"?>
...
<dar>
    ...
    <assets>${images
    .map(image => `<asset id="${image.filename.split(".")[0]}" mime-type="image/${image.filename.split(".")[1] === "png"
    ? "png"
    : image.filename.split(".")[1] === "svg"
        ? "svg+xml"
        : "jpeg"}" name="${image.title}" path="${image.filename}"/>`)
    .join("")}</assets>
</dar>`;

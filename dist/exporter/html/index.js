...
export class HTMLExporter {
    ...
    styleSheets;
    progressCallback;
    constructor(doc, bibDB, imageDB, csl, updated, documentStyles, converterOptions = {}, template = htmlExportTemplate, progressCallback) {
        this.doc = doc;
        this.bibDB = bibDB;
    ...
    async process() {
        ...
        this.converter = new HTMLExporterConvert(this.docTitle, this.doc.settings, this.docContent, this.htmlExportTemplate, this.imageDB, this.bibDB, this.csl, this.styleSheets, this.converterOptions);
        const { html, imageIds, metaData, extraStyleSheets } = await this.converter.init();
    ...
    addImages(imageIds) {
        imageIds.forEach(id => {
            ...
            if (imageValue instanceof Blob) {
                const ext = getImageExtension(image.file_type, imageValue.type);
                this.httpFiles.push({
    ...
    getDocStyle(doc) {
        ...
        docStyle.documentstylefile_set.forEach(([_url, filename]) => (contents = contents.replace(new RegExp(filename, "g"), `media/${filename}`)));
    ...
    async createZip() {
        const zipper = new ZipFileCreator(this.textFiles, this.httpFiles, this.includeZips, this.mimeType, this.updated);

...
export class HTMLExporterConvert {
    ...
    footnoteNumbering = "decimal", affiliationNumbering = "alpha", idPrefix = "", footnoteOffset = 0, affiliationOffset = 0, figureOffset = {}, trackChanges = false, mathOutput = "mathml" } = {}) {
    ...
    analyze(node) {
        ...
        switch (node.type) {
            ...
            case "doc":
                if (attrs.copyright) {
                    this.metaData.copyright = attrs.copyright;
                }
                break;
            case "heading1":
            case "heading2":
            ...
            case "footnote": {
                ...
                if (Array.isArray(footnote)) {
                    footnote.forEach((child) => this.analyze(child));
                }
                break;
            }
            case "richtext_part": {
    ...
    assembleHead() {
        ...
        if (typeof copyrightHolder === "string" && copyrightHolder.length) {
            ...
            const year = typeof copyright.year === "number"
                ? copyright.year
            ...
            if (Array.isArray(licenses)) {
                head += licenses
                    .map((license) => {
                    const licenseRecord = typeof license === "object" &&
                        license !== null
                        ? license
                        : {};
     ...
     */
    blockTrackData(attrs) {
        if (!this.trackChanges) {
            return "";
        }
        const track = attrs.track;
        if (Array.isArray(track)) {
            const type = track.find((t) => t &&
                typeof t.type === "string" &&
                t.type !== "block_change")?.type;
            if (type) {
        ...
        switch (node.type) {
                ...
                if (node.content) {
                    ...
                    node.content.forEach((childNode) => {
                        ...
                        if (firstname || lastname) {
                            ...
                            const nameParts = [];
                            if (typeof firstname === "string") {
                                nameParts.push(`<span class="firstname">${escapeText(firstname)}</span>`);
                            }
                            if (typeof lastname === "string") {
                                nameParts.push(`<span class="lastname">${escapeText(lastname)}</span>`);
                            }
                            if (nameParts.length) {
            ...
            case "code_block": {
                ...
                if (typeof language === "string") {
                    codeAttrs.push(`data-language="${escapeText(language)}"`);
                }
            ...
            case "blockquote":
                ...
                break;
            case "ordered_list": {
                const order = typeof attrs.order === "number" ? attrs.order : 1;
                if (order === 1) {
            ...
            case "footnotecontainer":
                ...
                break;
            case "text": {
                let strong, em, underline, hyperlink, anchor, sup, sub, code, insertion, deletion;
                ...
                if (node.marks) {
                    ...
                    insertion = node.marks.find((mark) => mark.type === "insertion");
                ...
                if (anchor) {
                    ...
                    const id = typeof anchorAttrs.id === "string" ? anchorAttrs.id : "";
            ...
            case "cross_reference": {
                const refId = typeof attrs.id === "string" ? attrs.id : "";
                const refTitle = typeof attrs.title === "string" ? attrs.title : "MISSING TARGET";
            ...
            case "figure": {
                ...
                const imageNode = figureContent.find((child) => child.type === "image");
                const image = imageNode?.attrs?.image || false;
                if (image !== false) {
                ...
                const caption = attrs.caption && figureContent.length
                    ? figureContent.find((child) => child.type === "figure_caption")?.content || []
                    : [];
                const figureCategory = typeof attrs.category === "string" ? attrs.category : "none";
                const figureId = typeof attrs.id === "string" ? attrs.id : "";
                const figureAligned = typeof attrs.aligned === "string" ? attrs.aligned : "";
                const figureWidth = typeof attrs.width === "string" ? attrs.width : "";
                if (figureCategory === "none" &&
                    ...
                    const equationNode = figureContent.find((child) => child.type === "figure_equation");
                    ...
                    if (caption.length || figureCategory !== "none") {
                        ...
                        if (caption.length) {
                            figcaption += `<p>${caption.map((child) => this.walkJson(child)).join("")}</p>`;
                        }
            ...
            case "table": {
                ...
                const tableWidth = typeof attrs.width === "string" ? attrs.width : "";
                const tableAligned = typeof attrs.aligned === "string" ? attrs.aligned : "";
                const tableLayout = typeof attrs.layout === "string" ? attrs.layout : "";
                const tableCategory = typeof attrs.category === "string" ? attrs.category : "none";
                start += `<table
            ...
            >`;
                ...
                const caption = attrs.caption && tableContent.length
                ...
                if (tableCategory !== "none" || caption.length) {
                    ...
                    if (caption.length) {
                        tableCaption += `<p>${caption.map((child) => this.walkJson(child)).join("")}</p>`;
                    }
            ...
            case "equation": {
                const equation = typeof attrs.equation === "string" ? attrs.equation : "";
                if (this.mathOutput === "svg") {

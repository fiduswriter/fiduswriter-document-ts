...
export class LatexExporterConvert {
        ...
        switch (node.type) {
                ...
                if (node.content) {
                    ...
                    if (node.attrs?.metadata === "authors") {
                        const authorsPerAffil = node.content
                            .map((authorNode) => {
                            const author = (authorNode.attrs || {}), nameParts = [];
                ...
                if (node.attrs?.category && node.attrs.id) {
                    ...
                    for (const n of descendantNodes(this.exporter.doc.content)) {
                        if (n.type === "code_block" &&
                            n.attrs?.category &&
                            n.attrs.id) {
                            const category = n.attrs.category;
            ...
            case "footnote": {
                ...
                if (options.onlyFootnoteMarkers) {
                    ...
                    if (!footnote.find((par) => par.type === "figure")) {
            ...
            case "figure": {
                ...
                if (category !== "none") {
                    ...
                    if (captionContent.length) {
                        caption = `${catLabel}: ${captionContent.map((n) => this.walkJson(n)).join("")}`;
                    }
                    else {
                        caption = catLabel;
                    }
                    ...
                    caption = captionContent
                        .map((n) => this.walkJson(n))
                        .join("");
                }
                let innerFigure = "";
                let copyright;
                const image = node.content
                    ?.find((n) => n.type === "image")
                    ?.attrs?.image || false;
                if (image) {
                    this.imageIds.push(image);
                    const imageDBEntry = this.imageDB.db[image], filename = getImageDBEntryFilename(imageDBEntry, image);
                    copyright =
                    ...
                    const equation = node.content
                        ?.find((n) => n.type === "figure_equation")
                        ?.attrs?.equation ?? "";
                    innerFigure += `\\begin{displaymath}\n${equation}\n\\end{displaymath}\n`;
                }
                if (category === "table") {
                    const aligned = node.attrs.width === "100"
                        ? "left"
                        : node.attrs.aligned;
                    if (aligned === "center") {
                        start += "\n\n\\begin{center}";
            ...
            case "table":
                if (node.content?.length) {
                    ...
                    if (node.content.length > 1 &&
                        ...
                        columns = node.content[1].content[0].content.reduce((columns, n) => columns +
                            (n.attrs?.colspan || 1), 0);
                    }
                    const aligned = node.attrs.width === "100"
                        ? "left"
            ...
            case "table_cell":
            case "table_header": {
                const colspan = node.attrs?.colspan ?? 0;
                const rowspan = node.attrs?.rowspan ?? 0;
                if (colspan > 1) {
    ...
    assembleEpilogue() {
        ...
        if (this.features.citations) {
            ...
            const bibliographyHeader = this.settings.bibliography_header?.[lang] ||
    ...
    assembleCopyright() {
        let note = "";
        const copyright = this.settings.copyright;
        if (copyright) {
            if (copyright.holder) {

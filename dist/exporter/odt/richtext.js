...
export class ODTExporterRichtext {
    ...
    images;
    imgCounter;
    fnCounter; // real footnotes
    fnAlikeCounter; // real footnotes and citations as footnotes
    categoryCounter; // counters for each type of table/figure category (figure/table/photo)
    fnCategoryCounter; // counters for each type of table/figure category (figure/table/photo)
    zIndex;
    bodyCitationCounter; // Track which body citation we're processing
    footnoteCitationCounter; // Track which footnote citation we're processing
        ...
        switch (node.type) {
                ...
                if (node.type === "code_block") {
                    ...
                    if (category && node.attrs.id) {
                        ...
                        const title = node.attrs.title
                            ...
                            : "";
                        const categoryLabel = `<text:bookmark-start text:name="${node.attrs.id}"/>${getCat(category, this.settings.language || "en-US")} ${catCountXml}${title}<text:bookmark-end text:name="${node.attrs.id}"/><text:line-break/>`;
                        start += categoryLabel;
                    }
                }
                break;
            ...
            case "citation": {
                ...
                const markName = citInfo && formattedText
                    ? createZoteroCitationMark(citInfo.references, citations.bibDB, formattedText)
                    : null;
            ...
            case "figure": {
                ...
                if (category !== "none") {
                    ...
                    const catCountXml = `<text:sequence text:ref-name="ref${category}${catCount - 1}${options.inFootnote ? "A" : ""}" text:name="${category}" text:formula="ooow:${category}+1" style:num-format="1">${catCount}${options.inFootnote ? "A" : ""}</text:sequence>`;
                    if (caption.length) {
                        caption = `<text:bookmark-start text:name="${node.attrs.id}"/>${getCat(category, this.settings.language || "en-US")} ${catCountXml}<text:bookmark-end text:name="${node.attrs.id}"/>: ${caption}`;
                    }
                    else {
                        caption = `<text:bookmark-start text:name="${node.attrs.id}"/>${getCat(category, this.settings.language || "en-US")} ${catCountXml}<text:bookmark-end text:name="${node.attrs.id}"/>`;
                    }
                }
            ...
            case "table_header":
                if (node.attrs.rowspan && node.attrs.colspan) {
                    start += `<table:table-cell${Number(node.attrs.rowspan) > 1

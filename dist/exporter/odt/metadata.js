...
export class ODTExporterMetadata {
    ...
    addMetadata() {
        ...
        if (titleEl) {
            ...
            metaEl.appendXML(`<dc:title>${escapeText(this.metadata.title)}</dc:title>`);
        }
        // Authors
        ...
        if (initialAuthorEl) {
            ...
            metaEl.appendXML(`<meta:initial-creator>${initialAuthor}</meta:initial-creator>`);
        }
        // Keywords
        // Remove all existing keywords
        const keywordEls = this.metaXml.queryAll("meta:keywords");
        keywordEls.forEach(keywordEl => keywordEl.parentElement.removeChild(keywordEl));
        // Add new keywords
        const keywords = this.metadata.keywords;
        keywords.forEach(keyword => metaEl.appendXML(`<meta:keyword>${escapeText(keyword)}</meta:keyword>`));
        // language
        ...
        if (languageEl) {
            ...
            metaEl.appendXML(`<dc:language>${this.metadata.language || "en-US"}</dc:language>`);
        }
        // time
        const date = new Date();
    ...
    async addZoteroPrefs() {
        ...
        const dataContent = escapeText(`<data data-version="3" zotero-version="8.0.2"><session id=""/><style id="${citationStyleUrl}" locale="${escapeText(this.metadata.language || "en-US")}" hasBibliography="${hasBib}" bibliographyStyleHasBeenSet="1"/><prefs><pref name="fieldType" value="ReferenceMark"/><pref name="automaticJournalAbbreviations" value="true"/></prefs></data>`);
    ...
    addContributorMetadata() {
        ...
        metaEl.appendXML(`<meta:user-defined meta:name="fidus_contributor_count" meta:value-type="float">${contributors.length}</meta:user-defined>`);

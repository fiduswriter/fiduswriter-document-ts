...
export const placeholderPlugin = (nodeTitle) => new Plugin({
    props: {
        decorations: (state) => {
            const doc = state.doc;
            if (doc.childCount === 1 &&
                ...
                placeHolder.setAttribute("data-placeholder", `${gettext("Add")} ${nodeTitle.toLowerCase()}...`);
        ...
        props: {
            handleDOMEvents: {
                paste(_view, event) {
                    ...
                    const html = clipboardData.getData("text/html"), text = clipboardData.getData("text/plain"), slice = text
                        .split(/[,;.]/)
                        .map(item => item.trim())
                    ...
                    if (text && !html) {
                        ...
                        const clipboardDoc = document.implementation.createHTMLDocument("paste document");
                        const pasteHTML = document.createElement("body");
                        clipboardDoc.body.appendChild(pasteHTML);
                        pasteHTML.innerHTML = html;
                        tags = Array.from(pasteHTML.querySelectorAll("span.tag")).map(tag => tag.textContent);
                    }
                    if (!tags.length) {
                        return;
                    }
...
//# sourceMappingURL=tag_editor_plugins.js.map
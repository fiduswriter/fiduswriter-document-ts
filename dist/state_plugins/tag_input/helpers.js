...
export const submitTag = (tagInputView, view, getPos) => {
    ...
    const tag = selectionTo > 1
        ? tagState.doc.textBetween(1, selectionTo)
        : tagState.doc.textContent;
    if (tag.length) {
        try {
            const eState = view.state, startPos = getPos();
            ...
            const pos = startPos + nodeAtPos.nodeSize - 1, node = eState.schema.nodes.tag.create({ tag });
            view.dispatch(view.state.tr.insert(pos, node));
            tagInputView.dispatch(tagState.tr.delete(1, selectionTo > 1 ? selectionTo : tagState.doc.nodeSize - 3));
        }
        catch (_e) {

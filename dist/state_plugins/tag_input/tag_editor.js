...
const ArrowLeft = (state, dispatch, getNode, view, getPos) => {
    ...
    if (state.selection.to > 1) {
        ...
        const tr = state.tr.setSelection(TextSelection.create(state.doc, state.selection.from - 1, state.selection.to - 1));
        dispatch(tr);
        return true;
    }
    else {
        ...
        if (node.nodeSize > 2) {
            ...
            view.dispatch(view.state.tr.setSelection(NodeSelection.create(view.state.doc, startPos)));
            view.focus();
            return true;
        }
        else {
...
const ArrowRight = (state, dispatch, getNode, view, getPos) => {
    ...
    if (state.selection.from < docSize) {
        ...
        const tr = state.tr.setSelection(TextSelection.create(state.doc, state.selection.from + 1, state.selection.from + 1));
        dispatch(tr);
        return true;
    }
    else {
...
const ArrowDown = (_state, _dispatch, getNode, view, getPos) => {
    ...
    const node = getNode();
    const startPos = getPos(), pos = startPos + node.nodeSize + 1;
        ...
        state: EditorState.create({
            ...
            plugins: [
                ...
                keymap({
                    ...
                    "Mod-shift-z": undo,
                    "Mod-y": redo,
                    Enter: ((_state, _dispatch, tagInputView) => submitTag(tagInputView, view, getPos)),
                    ArrowLeft: ((state, dispatch, _tagInputView) => ArrowLeft(state, dispatch, getNode, view, getPos)),
                    ArrowRight: ((state, dispatch, _tagInputView) => ArrowRight(state, dispatch, getNode, view, getPos)),
                    ArrowUp: ((state, dispatch, _tagInputView) => ArrowUp(state, dispatch, getNode, view, getPos)),
                    ArrowDown: ((state, dispatch, _tagInputView) => ArrowDown(state, dispatch, getNode, view, getPos))
                })
            ]
        }),
        handleDOMEvents: {
            ...
            focus: (tagInputView, _event) => {
                const startPos = getPos(), pos = startPos +
                    view.state.doc.nodeAt(startPos).nodeSize -
                    1, $pos = view.state.doc.resolve(pos);

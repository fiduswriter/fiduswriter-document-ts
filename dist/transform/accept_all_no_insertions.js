...
export function acceptAllNoInsertions(doc) {
    ...
    doc.descendants((node, pos) => {
        const deletionTrack = node.attrs.track
            ...
            : node.marks.find(mark => mark.type.name === "deletion"), insertionTrack = node.attrs.track
            ? node.attrs.track.find((track) => track.type === "insertion")
            : node.marks.find(mark => mark.type.name === "insertion"), formatChangeMark = node.marks.find(mark => mark.type.name === "format_change"), blockChangeTrack = node.attrs.track
        ...
        if (node.type.name === "footnote" && node.attrs.footnote) {
            ...
            }), cleanFnDoc = acceptAllNoInsertions(fnDoc), fnChange = !fnDoc.eq(cleanFnDoc);
            if (fnChange) {
                tr.setNodeMarkup(map.map(pos), null, Object.assign({}, node.attrs, {
...
//# sourceMappingURL=accept_all_no_insertions.js.map
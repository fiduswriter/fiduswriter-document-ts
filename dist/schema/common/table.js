    ...
    parseDOM: [
        {
            tag: "table",
            getAttrs(dom) {
                const track = parseTracks(dom.dataset.track), width = dom.dataset.width || "100", aligned = width === "100" ? "center" : dom.dataset.aligned || "center", layout = dom.dataset.layout || "fixed", id = dom.id || dom.dataset.id || false;
                return {
                    track,
                    width,
                    aligned,
